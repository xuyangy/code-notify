...
- **tmux badge trace** - To diagnose a window whose name and badge state disagree, enable tracing with `touch ~/.claude/notifications/tmux-badge-trace-enabled`. Badge apply/clear and running-indicator transitions are then written to `~/.claude/logs/tmux-badge-trace.log`. Tracing is off by default; remove the flag file to turn it off. The log rotates at about 512 KiB, keeping one older file at `tmux-badge-trace.log.1`.
...
| `cn spinner delegated off` | Keep the moon spinner during delegated work |
| `cn spinner delegated on` | Use the clock spinner during delegated work (default) |
...
cn spinner status
cn spinner off
cn spinner delegated off  # Keep the moon spinner when Claude delegates work
cn spinner delegated on   # Restore the clock spinner (default)
```
...
The delegated-work clock spinner is enabled by default when the animated spinner
is on. `cn spinner delegated off` saves a preference to keep the moon spinner
while Claude delegates work; `cn spinner delegated on` restores the clock spinner.
The toggle updates the current tmux server immediately. Use
`cn spinner delegated status` to show the saved preference.
...
- **omp** — `input` starts the running indicator, unless the text is a slash command: omp fires `input` before it dispatches those, and most (`/settings`, `/help`, `/plan`) are handled in the TUI with no agent turn behind them, so the indicator would stay up until some later turn ended. A slash input waits for `before_agent_start`, which fires only when a prompt actually reaches the agent loop — so `/skill:x`, `/loop` and custom commands still get an indicator and `/settings` does not. `tool_approval_requested` fires the approval alert (gated by the `permission_prompt` alert type), the `ask` tool fires `elicitation_dialog`, `tool_approval_resolved` brings the indicator back once you answer, and `agent_end` delivers task-complete or, for a turn that died, a failure alert. omp settles an interrupt through that same `agent_end`, so the outcome is read from the turn's last assistant message: cancelling with Escape retires the indicator silently instead of announcing a completion, the same way the tmux interrupt watch handles Claude and Codex.
