...
- **tmux window badges** - The originating tmux window's name gets the event icon prepended ("🏁 zsh"), so pending work is visible in the status line; the badge clears when you click the notification or visit the window. Disable with `CODE_NOTIFY_TMUX_BADGE=false` or `touch ~/.claude/notifications/tmux-badge-disabled`
