...
- **Input needed** — a `PreToolUse` hook fires before every tool call; the notifier banners only `run_command` calls that Antigravity's own permission lists won't auto-run, and only while the `permission_prompt` alert type is enabled.
